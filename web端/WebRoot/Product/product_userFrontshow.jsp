...
<div class="container">
	...
	<div class="row bottom15">
		...
		<div class="col-md-6 col-xs-6">
			<button onclick="lostApply('<%=product.getProductNo() %>','<%=product.getProState() %>')" class="btn btn-primary">申请挂失</button>
			<button onclick="jieDong('<%=product.getProductNo() %>','<%=product.getProState() %>')" class="btn btn-primary">申请解冻</button>
			<button onclick="reissue('<%=product.getProductNo() %>','<%=product.getProState() %>')" class="btn btn-primary">申请补办</button> 
...
//申请挂失
function lostApply(productNo,proState) {
	if(proState != "正常") {
		alert('非正常状态不能申请挂失');
		return;
	}
	location.href = "<%=basePath %>LostApply/lostApply_userFrontAdd.jsp?productNo=<%=product.getProductNo() %>";
	
}

//申请解冻
function jieDong(productNo,proState) {
	if(proState != "已挂失") {
		alert('只有挂失状态才能申请解冻');
		return;
	}
	
	location.href = "<%=basePath %>JieDong/jieDong_userFrontAdd.jsp?productNo=<%=product.getProductNo() %>";
}

//申请补办
function reissue(productNo,proState) {
	
	location.href = "<%=basePath %>Reissue/reissue_userFrontAdd.jsp?productNo=<%=product.getProductNo() %>";

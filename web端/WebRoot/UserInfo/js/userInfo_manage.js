...
function initUserInfoManageTool() {
	userInfo_manage_tool = {
		...
		search: function() {
			...
			queryParams["schoolState"] = $("#schoolState").val();
				...
				$.ajax({
					...
					success : function (userInfo, response, status) {
						...
						if (userInfo) { 
							...
							$("#userInfo_schoolState_edit").validatebox({
								required : true,
								missingMessage : "请输入是否在校",
							});
							$("#userInfo_roleName_edit").val(userInfo.roleName);
							$("#userInfo_roleName_edit").validatebox({
								required : true,
								missingMessage : "请输入用户身份",
							});

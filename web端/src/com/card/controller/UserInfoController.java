...
public class UserInfoController extends BaseController {
	...
	public void list(String user_name,String name,String birthDate,String telephone,String schoolState,String roleName,Integer page,Integer rows, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
		if (page==null || page == 0) page = 1;
		if (user_name == null) user_name = "";
		if (name == null) name = "";
		if (birthDate == null) birthDate = "";
		if (telephone == null) telephone = "";
		if (schoolState == null) schoolState = "";
		if (roleName == null) roleName = "";
		if(rows != 0)userInfoService.setRows(rows);
		List<UserInfo> userInfoList = userInfoService.queryUserInfo(user_name, name, birthDate, telephone, schoolState, roleName, page);
	    /*计算总的页数和总的记录数*/
	    userInfoService.queryTotalPageAndRecordNumber(user_name, name, birthDate, telephone, schoolState, roleName);
	    /*获取到总的页码数目*/
	...
	public String frontlist(String user_name,String name,String birthDate,String telephone,String schoolState,String roleName,Integer currentPage, Model model, HttpServletRequest request) throws Exception  {
		...
		if (schoolState == null) schoolState = "";
		if (roleName == null) roleName = "";
		List<UserInfo> userInfoList = userInfoService.queryUserInfo(user_name, name, birthDate, telephone, schoolState, roleName, currentPage);
	    /*计算总的页数和总的记录数*/
	    userInfoService.queryTotalPageAndRecordNumber(user_name, name, birthDate, telephone, schoolState, roleName);
	    /*获取到总的页码数目*/
	    int totalPage = userInfoService.getTotalPage();
	    ...
	    request.setAttribute("schoolState", schoolState);
	...
	public void OutToExcel(String user_name,String name,String birthDate,String telephone,String schoolState,String roleName, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
        if(user_name == null) user_name = "";
        if(name == null) name = "";
        if(birthDate == null) birthDate = "";
        if(telephone == null) telephone = "";
        if(schoolState == null) schoolState = "";
        if(roleName == null) roleName = "";
        List<UserInfo> userInfoList = userInfoService.queryUserInfo(user_name,name,birthDate,telephone,schoolState,roleName);
        ExportExcelUtil ex = new ExportExcelUtil();
        String _title = "UserInfo信息记录"; 
        String[] headers = { "用户名","姓名","性别","出生日期","用户照片","联系电话","邮箱","是否在校","用户身份","注册时间"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<userInfoList.size();i++) {
        	UserInfo userInfo = userInfoList.get(i); 
        	dataset.add(new String[]{userInfo.getUser_name(),userInfo.getName(),userInfo.getGender(),userInfo.getBirthDate(),userInfo.getUserPhoto(),userInfo.getTelephone(),userInfo.getEmail(),userInfo.getSchoolState(),userInfo.getRoleName(),userInfo.getRegTime()});
        }
        /*
